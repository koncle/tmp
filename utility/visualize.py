...
def line_plot(data, x_axis_names=None, line_names=None,
              marker_size=35, text_size=15, line_width=3,
              text_x_offset=-0.15, text_y_offset=0.05, colors=None,
              x_label=None, y_label=None, title=None, ylim=None, yticks=None, figsize=(8, 5),
              title_size=15, label_size=15, tick_size=15, legenc_loc=None, legend_size=15, ytick_format_n=None,
              save_name=None, x_axis_name_rotate=None, plot_text=False, plot_scatter=False, grid=False, show=True,
              vertical_line_configs=None):
    # vertical_line_configs = {y=79.44, color='black', linestyle='-', zorder=0, label='baseline'}
    ...
    if vertical_line_configs is not None:
        [plt.gca().axvline(**config) for config in vertical_line_configs]
    ...
    x_axis=None if x_axis_names is None else np.arange(0, len(data[0]), len(data[0])//(len(x_axis_names)-1)-1)
...
def bar_plot(data, x_axis_names=None, bar_names=None,
    ...
    plt.figure(figsize=figsize)
    if horizontal_line_configs is not None:
        [plt.gca().axhline(**config) for config in horizontal_line_configs]
        # y=79.44, color='black', linestyle='-', zorder=0, label='baseline'
    if grid:
        plt.grid(axis='y')
        plt.gca().set_axisbelow(True)
