...
class Log:
    def __init__(self, log_each: int, initial_epoch=-1, filename=''):
        self.loading_bar = LoadingBar(length=27)
        self.best_accuracy = 0.0
        self.log_each = log_each
        self.epoch = initial_epoch
        self.filename = filename + time.strftime("/%Y-%m-%d_%H-%M-%S.txt")
    ...
    def train(self, len_dataset: int) -> None:
        ...
        self.is_train = True
        self.last_steps_state = {"loss": 0.0, "accuracy": 0.0, "alphas": 0}
    ...
    def flush(self, show=True) -> None:
        if self.is_train:
            loss = self.epoch_state["loss"] / self.epoch_state["alphas"]
            accuracy = self.epoch_state["accuracy"] / self.epoch_state["alphas"]
            ...
            if show:
                ...
                with open(self.filename, 'a') as f:
                    f.write( f"┃{self.epoch:12d}  ┃{loss:12.4f}  │{100 * accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃")
            ...
            if show:
                ...
                with open(self.filename, 'a') as f:
                    f.write(f"{loss:12.4f}  │{100 * accuracy:10.2f} %  ┃\n")
    ...
    def _train_step(self, model, loss, accuracy, learning_rate: float) -> None:
        ...
        self.last_steps_state["alphas"] += loss.size(0)
        self.epoch_state["loss"] += loss.sum().item()
        self.epoch_state["accuracy"] += accuracy.sum().item()
        self.epoch_state["alphas"] += loss.size(0)
        self.step += 1

        if self.step % self.log_each == self.log_each - 1:
            loss = self.last_steps_state["loss"] / self.last_steps_state["alphas"]
            accuracy = self.last_steps_state["accuracy"] / self.last_steps_state["alphas"]
    ...
    def _reset(self, len_dataset: int) -> None:
        ...
        self.epoch_state = {"loss": 0.0, "accuracy": 0.0, "alphas": 0}
    ...
    def _print_header(self) -> None:
        ...
        print(f"┠──────────────╂──────────────┼──────────────╂──────────────┼──────────────╂──────────────┼──────────────┨")
        with open(self.filename, 'a') as f:
            f.write(f"┏━━━━━━━━━━━━━━┳━━━━━━━╸T╺╸R╺╸A╺╸I╺╸N╺━━━━━━━┳━━━━━━━╸S╺╸T╺╸A╺╸T╺╸S╺━━━━━━━┳━━━━━━━╸V╺╸A╺╸L╺╸I╺╸D╺━━━━━━━┓\n")
            f.write(f"┃              ┃              ╷              ┃              ╷              ┃              ╷              ┃\n")
            f.write(f"┃       epoch  ┃        loss  │    accuracy  ┃        l.r.  │     elapsed  ┃        loss  │    accuracy  ┃\n")
            f.write(f"┠──────────────╂──────────────┼──────────────╂──────────────┼──────────────╂──────────────┼──────────────┨\n")
